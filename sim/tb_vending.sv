        ...
        module tb_vending;
            ...
            task check(input logic [WIDTH-1:0] exp, input logic [WIDTH-1:0] act, input string name);
                if (exp === act)
                    ...
                    $display("[FAIL] %s | exp=%0d act=%0d", name, exp, act);
            endtask
            ...
            initial begin
                ...
                @(posedge clk);
                while (rst) @(posedge clk) begin end
                repeat (3) @(posedge clk) begin end
                ...
                rst = 1'b1; #10; rst = 1'b0; repeat (3) @(posedge clk) begin end

                //-------------------------------------
                // CENÁRIO 2: Crédito Insuficiente
                //-------------------------------------
                $display("\n--- CENARIO 2 ---");
                #1 sel_item = 2'b11; // Snack (R$ 1,00)
                coin_in  = 2'b01;    // Insere apenas R$ 0,25 -> COLLECT
                ...
                rst = 1'b1; #10; rst = 1'b0; repeat (2) @(posedge clk) begin end
        ...
        //         moeda 1
            ...
            // Dá folga de clock para fixar o valor total de 200 de forma estável
                ...
                repeat (5) begin
                    #1 coin_in = 2'b11; repeat (2) @(posedge clk) begin end
                    #1 coin_in = 2'b00; repeat (2) @(posedge clk) begin end
                    confirm = 1'b1;
                    @(posedge clk);
                    #1 confirm = 1'b0; repeat (6) @(posedge clk) begin end
                end
